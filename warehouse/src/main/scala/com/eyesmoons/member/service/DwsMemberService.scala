...
import com.eyesmoons.member.bean.{DwsMember, DwsMember_Result, MemberZipper, MemberZipperResult}
import com.eyesmoons.member.dao.DwdMemberDao
...
object DwsMemberService {
  ...
  def importMember(sparkSession: SparkSession, time: String) = {
    ...
    val reuslt = dayResult.union(historyResult).groupByKey(item => item.uid + "_" + item.dn)
      ...
      }.flatMap(_.list).write.mode(SaveMode.Overwrite).insertInto("dws.dws_member_zipper") //重组对象打散 刷新拉链表
