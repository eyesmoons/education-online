...
import com.eyesmoons.member.dao.DwsMemberDao
...
object AdsMemberService {
    ...
    def queryDetailApi(sparkSession: SparkSession, dt: String): Unit = {
        ...
        result.mapPartitions(partition => {
            ...
            .map(item=>{
                val arr: Array[String] = item._1.split("_")
                (arr(0),item._2,arr(1),arr(2))
            }).toDF().coalesce(1).write.mode(SaveMode.Overwrite).insertInto("ads.ads_register_appregurlnum")

        //统计所属网站人数
        result.mapPartitions(partition=>{
            partition.map(item=>(item.sitename + "_" + item.dt + "_" + item.dn,1))
        }).groupByKey(_._1)
        ...
        result.mapPartitions(par=>{
            par.map(item=>{
                (item.regsourcename + "_" + item.dt + "_" + item.dn,1)
            })
        }).groupByKey(_._1)
            .mapValues(_._2).reduceGroups(_+_)
            .map(item=>{
                val arr: Array[String] = item._1.split("_")
                (arr(0),item._2,arr(1),arr(2))
            }).toDF().write.mode(SaveMode.Overwrite).insertInto("ads.ads_register_regsourcenamenum")
