package com.eyesmoons.qz.service

import com.eyesmoons.qz.dao.{QzChapterDao, QzCourseDao, QzMajorDao, QzPaperDao, QzQuestionDao, UserPaperDetailDao}
...
object DwsQzService {
    ...
    def saveDwsQzMajor(sparkSession: SparkSession, dt: String) = {
        ...
        val result: DataFrame = dwdQzMajor.join(dwdQzWebsite, Seq("siteid", "dn")).join(dwdQzBusiness, Seq("businessid", "dn"))
        ...
        result.toDF().write.mode(SaveMode.Append).insertInto("dws.dws_qz_major")
    }
    ...
    def saveDwsQzChapter(sparkSession: SparkSession, dt: String) = {
        sparkSession.sql("set hive.exec.dynamic.partition.mode=non-strict")
