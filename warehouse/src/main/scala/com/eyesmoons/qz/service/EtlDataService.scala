...
import com.eyesmoons.qz.bean.{DwdQzPaperView, DwdQzPoint, DwdQzQuestion}
import com.eyesmoons.util.ParseJsonData
...
object EtlDataService {
    ...
    def etlQzChapter(ssc: SparkContext, sparkSession: SparkSession) = {
        ...
        ssc.textFile("/user/atguigu/ods/QzChapter.log").filter(item=>{
            val json: JSONObject = ParseJsonData.getJsonData(item)
        ...
        }).toDF().write.mode(SaveMode.Append).insertInto("dwd.dwd_qz_chapter")
    }
