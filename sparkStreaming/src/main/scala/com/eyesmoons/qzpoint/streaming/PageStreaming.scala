...
import com.eyesmoons.qzpoint.utils.{DataSourceUtil, ParseJsonData, QueryCallback, SqlProxy}
...
object PageStreaming {
    ...
    def main(args: Array[String]): Unit = {
        ...
        //解析json数据
        val dstream: DStream[(String, String, String, String, String, String, String)] = stream.map(item => item.value()).mapPartitions(partition => {
            partition.map(item => {
        ...
        dstream.cache()
        val pageValueDstream: DStream[(String, Int)] = dstream.map(item => (item._5 + "_" + item._6 + "_" + item._7, 1))
        ...
        val ipDstream: DStream[(String, Long)] = dstream.mapPartitions(patitions => {
