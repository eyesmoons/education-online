...
import com.eyesmoons.qzpoint.bean.LearnModel
...
object CourseLearnStreaming {
    ...
    def main(args: Array[String]): Unit = {
        val sparkConf: SparkConf = new SparkConf().setAppName(this.getClass.getSimpleName)
            ...
            .set("spark.streaming.kafka.backPressure", "true")
            .setMaster("local[*]")
        ...
        dsStream.foreachRDD(rdd=>{
            ...
            val groupByRdd: RDD[(String, Iterable[LearnModel])] = rdd.groupBy(item => item.userId + "_" + item.cwareId + "_" + item.videoId)
            groupByRdd.foreachPartition(partition=>{
                val sqlProxy = new SqlProxy()
                val client = DataSourceUtil.getConnection
                try {
